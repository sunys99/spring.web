...
public class DatabaseMessageSource extends AbstractMessageSource implements
	...
	public void afterPropertiesSet() throws Exception {
		try {
			...
			// 2. get cache
			cache = cacheMessageSource.getCache();
	...
	protected String resolveCodeWithoutArguments(String code, Locale locale) {
		...
		String cacheKey = code + "_" + locale.getLanguage();
		...
		if (result == null || result.equals(UNDEFINED)) {
			...
			cacheKey = code + "_" + defaultLocale.getLanguage();
					...
					public void setValues(PreparedStatement ps, int i)
						...
						if (!lazyLoad) {
							cache.put(new Element(message.getKey() + "_"
									+ message.getLanguage(), message.getText()));
						}
					}
	...
	private List<Message> readMessages(final boolean caching) {
		...
		List<Message> phrase = jdbcTemplate.query(sql.toString(),
				new RowMapper<Message>() {
					public Message mapRow(ResultSet rs, int idx)
						...
						if (caching) {
							Element element = new Element(keyValue + "_" + languageValue, textValue);
							cache.put(element);
						}

						return new Message(keyValue, languageValue, textValue);
					}
				});
	...
	private String readMessage(String code, Locale locale) {
		...
		String result = null;
		try {
			result = jdbcTemplate.queryForObject(sql.toString(), new Object[] {	code, locale.getLanguage()},
			...
			Element element = new Element(code + "_" + locale.getLanguage(), UNDEFINED);
			cache.put(element);
			return UNDEFINED;
		}
